...
if (isset($_POST['submit'])) {
  if((!empty($_POST['title'])) && (!empty($_POST['message']))) {
    ...
    $query = "INSERT INTO lesson1 (title, message, date) VALUES ('".$_POST['title']."', '".$_POST['message']."', '".date('Y-m-d H:i:s')."');";
...
<div class="rightal"><a href="reset_db.php?source=xss_1" title="Resetta Database">Reset Database</a></div>
...
<form action="index.php?p=xss_1" method="POST">
...
<table class="messageboard">
  ...
  <?php
  $values = extract_from_db('lesson1', 'date');
  if (!empty($values)) {
